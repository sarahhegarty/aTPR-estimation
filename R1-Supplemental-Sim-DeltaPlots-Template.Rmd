...
folder = paste0('/Users/hegarty/Library/CloudStorage/Box-Box/Research/Dissertation/12 Adjusted TPR/numerical/',params$simid,'/simout/')
bench = paste0('/Users/hegarty/Library/CloudStorage/Box-Box/Research/Dissertation/12 Adjusted TPR/numerical/',params$simid,'/r1out/')
...
for(k in 1:60){
  ...
  load(paste0(bench,'dcomps-ref1-GAM-QQ-CF-setting-',k,'.RData'))
    #benchset <- attributes(runk)[["n1"]]
...
stack.sum <- diffs %>%
            mutate(estimate = if_else(est == 'adjusted','Adjusted','Naive')
                  ...
                  ,delta = if_else(refgp ==1, delta, -delta)) %>%
...
ggsave(paste0("supfigs/R1-dTPR-Forest_",simlab,"-",toupper(substr(params$h2,1,1)),params$sigeps*10,".eps"),height=6.5,width=6, dpi = 300)
```
